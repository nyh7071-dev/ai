...
export default function ProjectNewPage() {
  ...
  const onUploadTemplate = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      router.push(
        `/project/new/result?type=${encodeURIComponent(type)}&templateId=${encodeURIComponent(templateId)}`
      );
    } catch (err) {
      console.error(err);
  ...
  const goDefault = () => {
    router.push(`/project/new/result?type=${encodeURIComponent(type)}`);
  };
    ...
    <div style={{ padding: 28, maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ padding: 16, border: "1px solid #e2e8f0", borderRadius: 12, background: "#fff" }}>
        ...
        <div style={{ marginTop: 12, color: "#475569", fontSize: 13 }}>
          {busy ? "처리 중..." : "DOCX 업로드는 여기서, PDF 업로드는 다음 화면에서 합니다."}
        </div>
