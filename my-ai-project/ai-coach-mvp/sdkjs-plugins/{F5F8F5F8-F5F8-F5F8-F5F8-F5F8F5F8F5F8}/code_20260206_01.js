...
(function (window, undefined) {
  ...
  // ============================================
  // PostMessage ping (cross-origin 통신)
  // window.top 우선, fallback으로 parent
  // ============================================
  function ping(type, payload) {
    ...
    // top으로 전송 (Next.js가 있는 최상위 window)
    try {
      if (window.top) {
        window.top.postMessage(msg, "*");
        console.log("[AI Assistant][PING->TOP]", type, payload);
      }
    } catch (e) {
      console.warn("[AI Assistant][PING->TOP ERROR]", e);
    }

    // fallback: parent로도 전송
    try {
      if (window.parent && window.parent !== window.top) {
        window.parent.postMessage(msg, "*");
        console.log("[AI Assistant][PING->PARENT]", type, payload);
      }
    } catch (e) {
      console.warn("[AI Assistant][PING->PARENT ERROR]", e);
    }
  }

  // ============================================
  // 즉시 boot 신호 전송 (플러그인 파일 로드 확인)
  // ============================================
  ping("boot", { href: location.href, buildId: __BUILD_ID__ });

  // ============================================
  // 에러 핸들러 (에러도 postMessage로 전송)
  // ============================================
  window.addEventListener("error", function (e) {
    console.error("[AI Assistant][ERROR]", e.message, e.error);
    ping("error", {
      message: e.message,
      filename: e.filename,
      lineno: e.lineno,
  ...
  window.addEventListener("unhandledrejection", function (e) {
    console.error("[AI Assistant][REJECTION]", e.reason);
    ping("error", {
      message: "Unhandled Promise Rejection",
      reason: e.reason && e.reason.toString ? e.reason.toString() : String(e.reason),
  ...
  window.Asc.plugin.init = function (initData) {
    ...
    // Parent에 플러그인 실행 신호 전송
    ping("init", { href: location.href });

    // onDocumentContentReady 이벤트 등록 (config.json events 목록과 대응)
    window.Asc.plugin.attachEditorEvent("onDocumentContentReady", function () {
      console.log("[AI Assistant] onDocumentContentReady fired — 문서 콘텐츠가 준비됨");
      ping("docReady", {});
    });
